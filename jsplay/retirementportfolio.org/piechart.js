...
function UserInputViewModel() {
	...
	self.formattedAsset = ko.computed({
		read: function(){
			return Number(this.asset() * 1000).toLocaleString()
		},
		write: function(value){
			this.asset(Number(value.replace(/,/g, '')) / 1000);
		},
		owner: this
	})
	...
	self.formattedMonthlySavings = ko.computed({
		read: function(){
			return Number(this.monthlySavings()).toLocaleString()
		},
		write: function(value){
			this.monthlySavings(Number(value.replace(/,/g, '')) );
		},
		owner: this
	})

	self.monthlyRetirementIncome = ko.observable(5000, { persist: 'monthlyRetirementIncome' });
	self.formattedMonthlyRetirementIncome = ko.computed({
		read: function(){
			return Number(this.monthlyRetirementIncome()).toLocaleString()
		},
		write: function(value){
			this.monthlyRetirementIncome(Number(value.replace(/,/g, '')) );
		},
		owner: this
	})
...
$(function () {
	...
	// Only allow numbers to be typed in number input boxes
	$('input.number-with-comma').on('keyup', function (e) {
		// Allow: backspace, delete, tab, escape, enter, comma
		if ($.inArray(e.keyCode, [46, /*8,*/ 9, 27, 13, 110]) !== -1 ||
			// Allow: Ctrl+A
			(e.keyCode == 65 && e.ctrlKey === true) ||
		...
		// Ensure that it is a number and stop the keypress
		if (e.keyCode != 8 && e.keyCode != 188 && (e.shiftKey || (e.keyCode < 48 || e.keyCode > 57)) && (e.keyCode < 96 || e.keyCode > 105)) {
			e.preventDefault();
		}

		// Format the number with comma
		var numberVal = Number($(this).val().replace(/,/g, ''))
		if(numberVal > 2000000) numberVal = 2000000
		$(this).val(numberVal.toLocaleString())	
	});	
	...
	$('input.number-with-comma').on('input', {target: 'input-box'}, handleSlideStop);
